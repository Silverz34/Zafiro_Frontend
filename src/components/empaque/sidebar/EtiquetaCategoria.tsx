...
export default function Etiquetas({ desactivadas, onToggleEtiqueta }: EtiquetaProp) {
  ...
  const { etiquetas, loading, agregarEtiqueta, editarEtiqueta, borrarEtiqueta } = useEtiquetasCtx();
  ...
  const handleEliminar = async (etiqueta: EtiquetaFrontend) => {
    ...
    if (!estabaDesactivada) {
      onToggleEtiqueta(String(etiqueta.id));
    }
  };
  ...
  return (
    <div className="w-full px-1">
      <div className="flex items-center justify-between mb-4">
        ...
        {!loading && (
          <button
            onClick={() => { setEtiquetaEditar(null); setModalAbierto(true); }}
      ...
      <div className="space-y-2.5">
        {loading ? (
          <>
            <Skeleton className="h-10 w-full rounded-lg bg-[#1e1d3a]" />
            <Skeleton className="h-10 w-[85%] rounded-lg bg-[#1e1d3a]" />
            <Skeleton className="h-10 w-[70%] rounded-lg bg-[#1e1d3a]" />
          </>
        ) : (
          etiquetas.map((etiqueta) => {
            ...
            return (
              ...
              >
                <button className="flex items-center justify-center w-5 h-5 rounded-full bg-white/80 border border-blue-600 shrink-0 transition-all">
