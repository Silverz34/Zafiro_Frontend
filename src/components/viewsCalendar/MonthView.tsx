                ...
                {days.map((date, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            onClick={onOpenModal}
                            className={`border-r border-b border-gray-800 p-1 flex flex-col gap-1 min-h-25 hover:bg-gray-800/30 transition-colors cursor-pointer ${!isCurrentMonth ? 'bg-[#0a0914] opacity-50' : ''
                                }`}
                        >
                            <div className="flex justify-center mb-1">
                                <span className={`text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full mt-1 ${isToday ? 'bg-blue-600 text-white font-bold shadow-md shadow-blue-900/50' : 'text-gray-300'
                                    }`}>
                                    {date.getDate()}
                                </span>
                            </div>
